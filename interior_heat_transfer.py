...
class InternalAdaptiveConvection:
    ...
    def calculate_h_c(self, surface, zone_air_temp_c):
        """
        Calculates the interior convective heat transfer coefficient (h_c).
        """
        classification = self._classify_surface(surface, zone_air_temp_c)
        ...
        return self._calculate_h(model_name, surface, zone_air_temp_c)
        
    def _classify_surface(self, surface, zone_air_temp_c):
        """Determines the surface classification."""
        surface_temp = surface['solver'].nodes[0]['T']
        ...
        if 90 > tilt > 0 or 180 > tilt > 90: # Tilted Surface
            ...
            surface_type = surface['type'].lower()
            if delta_t > 0: # Heated surface
    ...
    def _calculate_h(self, model_name, surface, zone_air_temp_c):
        ...
        return dispatch_map[model_name](delta_t_abs, surface['tilt'])
