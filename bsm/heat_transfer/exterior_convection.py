...
from bsm.core.constants import ROUGHNESS_MULTIPLIERS
...
class AdaptiveConvectionAlgorithm:
    ...
    def __init__(self, hf_model_assignments, hn_model_assignments):
        ...
        Args:
            hf_model_assignments (dict): A dictionary mapping surface classifications
                ...
                Available forced convection models:
                - 'SparrowWindward', 'SparrowLeeward': Sparrow et al. (1979)
                - 'BlockenWindward': Blocken et al. (2009)
                - 'EmmelVertical', 'EmmelRoof': Emmel et al. (2007)
                - 'Mitchell': Mitchell (1976)

                Combined models (include natural convection):
                - 'MoWITTWindward', 'MoWITTLeeward': Yazdanian & Klems (1994)
                - 'DOE2Windward', 'DOE2Leeward': DOE-2 algorithm (MoWiTT forced + TARP natural)
                - 'NusseltJurges', 'McAdams': Classic correlations
        ...
        self._combined_models = [
            ...
            'McAdams', 'DOE2Windward', 'DOE2Leeward'
        ]
    ...
    def _calculate_h_combined(self, model_name, surface, weather):
        ...
        models = {
            ...
            'DOE2Windward': self._hc_doe2_windward,
            'DOE2Leeward': self._hc_doe2_leeward,
        }
    ...
    def _hc_doe2_windward(self, surface, weather):
        """
        DOE-2 algorithm for windward exterior surfaces.

        Combines MoWiTT forced convection with TARP natural convection using
        root-sum-of-squares combination. This is the approach used by DOE-2
        and EnergyPlus for detailed exterior convection calculations.

        Reference: Lawrence Berkeley Laboratory DOE2.1E-053
                   EnergyPlus Engineering Reference, Exterior Convection
        """
        # MoWiTT windward forced convection: Yazdanian & Klems (1994)
        v_z = weather['wind_speed_local_ms']
        hf = 3.26 * (v_z ** 0.89)

        # TARP natural convection
        delta_t = abs(surface['surface_temp_c'] - weather['air_temp_c'])
        # Use Walton correlation for natural convection
        hn = 1.31 * (delta_t ** (1./3.)) if delta_t > 0 else 0.0

        # Root-sum-of-squares combination (EnergyPlus approach)
        # H_total = sqrt(Hn² + Hf²)
        return math.sqrt(hn**2 + hf**2)

    def _hc_doe2_leeward(self, surface, weather):
        """
        DOE-2 algorithm for leeward exterior surfaces.

        Combines MoWiTT leeward forced convection with TARP natural convection
        using root-sum-of-squares combination.

        Reference: Lawrence Berkeley Laboratory DOE2.1E-053
                   EnergyPlus Engineering Reference, Exterior Convection
        """
        # MoWiTT leeward forced convection: Yazdanian & Klems (1994)
        v_z = weather['wind_speed_local_ms']
        hf = 3.55 * (v_z ** 0.617)
