...
def run_simulation_from_config(config_path):
    ...
    try:
        ...
        # Additional arrays for LLM tracking
        ...
        for t in range(num_steps):
            ...
            # Base internal gains from schedule
            current_internal_gains = internal_gains_profile[t]

            # Add equipment, lighting, and metabolic heat if using LLM agents
            if use_llm_agents and llm_manager is not None:
                # get_total_internal_gains_w() includes:
        ...
        try:
            ...
            # Prepare data columns - include equipment/lighting/occupants if LLM mode
            if use_llm_agents:
        ...
        # 3. Calculate comprehensive energy metrics
        total_equipment_kwh = np.sum(equipment_power_profile) * dt_sec / (3600 * 1000) if use_llm_agents else 0.0
        total_lighting_kwh = np.sum(lighting_power_profile) * dt_sec / (3600 * 1000) if use_llm_agents else 0.0
        ...
        peak_equipment_w = float(np.max(equipment_power_profile)) if use_llm_agents else 0.0
        peak_lighting_w = float(np.max(lighting_power_profile)) if use_llm_agents else 0.0
